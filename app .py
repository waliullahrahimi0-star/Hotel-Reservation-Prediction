...
def load_and_prepare_data():
    ...
    # create the binary target: 1 = cancelled, 0 = not cancelled
    df["target"] = (df["booking_status"] == "Canceled").astype(int)
    # I engineered two extra columns that turned out to be useful signals
...
def build_explanation(lead_t, special_req, avg_usd, prev_cancel,
    ...
    if lead_t > 150:
        drivers.append("a long lead time")
    elif lead_t > 60:
        drivers.append("a mediocre lead time")
    elif lead_t < 14:
    ...
    if bk_type == "Online":
        drivers.append("an online booking channel, which historically carries the high cancellation rates")
    elif bk_type in ["Corporate", "Offline"]:
        drivers.append(f"a {bk_type.lower()} booking channel, associated with more committed to reservations")
    ...
    # fallback if none of the specific conditions fire
    if not drivers:
    ...
    if cancel_prob >= CANCEL_THRESHOLD:
        return (f"This prediction is initially influenced by the {body_str}. "
        ...
        return (f"This prediction benefits from {body_str}. "
                "Together, these factors are linked with reservations that "
                "are more likely to proceed to check-in.")


def get_key_drivers(lead_t, special_req, avg_usd, prev_cancel,
                    bk_type, ret_guest, total_n, cancel_prob):
    # Returns up to 4 bullet points for the Key Drivers section.
    # Each entry is a (direction, text) tuple where direction is
    # one of: 'up', 'down', or 'neut'
    avg_eur = avg_usd / EUR_TO_USD
    drivers = []

    if lead_t > 100:
        drivers.append(("up", "Long lead time also increased the cancellation risk"))
    elif lead_t < 14:
        drivers.append(("down", "Short lead time may reduced cancellation risk"))
    else:
        drivers.append(("neut", "neutral lead time had a neutral effect"))
    ...
    if special_req == 0:
        ...
        drivers.append(("neut", "One special request had a slight positive effect"))

    if prev_cancel >= 1:
        drivers.append(("up", f"{prev_cancel} prior cancellation increased risk"))
    elif ret_guest == "Yes":
...
def get_impact_text(feature_label, raw_value):
    """Plain text impact label shown in the summary table at the bottom."""
    try:
...
def render_feature_chart(top_feats):
    ...
    # highlight the top feature in darker blue, rest in lighter shade
...
if predict_button:
    ...
    # light sanity checks before we pass anything to the model
    ...
    # convert display values back to what the model was trained on
